...
import { getLocalParticipant ,isLocalParticipantModerator,getModerator} from '../../base/participants/functions';
...
import { getLargeVideoParticipant,getLargeModeratorVideo } from '../functions';
...
interface IProps {
    ...
    _largeVideoModeratorId:string


    _isModerator:boolean;


    _webinarEnabled:boolean;
...
class LargeVideo extends Component<IProps> {
    ...
    componentDidUpdate(prevProps: IProps) {
        ...
        if(!_webinarEnabled){
            ...
            console.log('wwwwwwwwwwwwwwww',_webinarEnabled);
    if(_webinarEnabled && !_isModerator) {
    
         console.log('largeVideoModeratorId ..........',_largeVideoModeratorId )
                VideoLayout.updateLargeVideo(  _largeVideoModeratorId , true, true);
...
function _mapStateToProps(state:IReduxState) {
    ...
    const isModerator=isLocalParticipantModerator(state)
       ...
       // const moderatorId=getModerator(state)?.id
       // console.log('MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM',moderatorId)
       const moderator=getModerator(state);
       
   const moderatorId = moderator.length > 0 ? moderator[0].id : null;
   const largeVideoModerator=getLargeModeratorVideo(state)
   
       // moderatorId=getModerator(state)?.idy
       const moderatorIds = moderator.map(moderator => moderator.id);
   ...
   //console.log('webbbbbbb
    return {
        ...
        _largeVideoModeratorId:largeVideoModerator?.id ??'',
