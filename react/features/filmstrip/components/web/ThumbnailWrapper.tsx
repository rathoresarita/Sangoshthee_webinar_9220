...
import { getLocalParticipant, getModerator,isLocalParticipantModerator} from '../../../base/participants/functions';
...
function _mapStateToProps(state: IReduxState, ownProps: { columnIndex: number;
    ...
    //const isTileViewEnabled = shouldDisplayTileView(state);
    ...
    if (_currentLayout === LAYOUTS.TILE_VIEW || _verticalViewGrid || stageFilmstrip) {
        ...
        if (!webinarEnabled&&stageFilmstrip) {
            return {
        ...
        else if(!isModerator && webinarEnabled){
            return {
                _disableSelfView: disableSelfView,
                _filmstripType: filmstripType,
                _participantID: moderatorIds[index],
                _horizontalOffset: horizontalOffset,
                _thumbnailWidth: thumbnailWidth
            };
        }
        ...
        if (!webinarEnabled &&!iAmRecorder && index === localIndex) {
            return {
                _disableSelfView: disableSelfView,
                _filmstripType: filmstripType,
        ...
        }else if(!isModerator &&!iAmRecorder && index === localIndex){
            return {
                _filmstripType: filmstripType,
                _participantID: moderatorIds[index],
                _horizontalOffset: horizontalOffset,
        ...
        else if(isModerator && webinarEnabled && !iAmRecorder && index === localIndex  ){
            return {
                _disableSelfView: disableSelfView,
                _filmstripType: filmstripType,
                _participantID: 'local',
...
//     };
// }else if(!isModerator && webinarEnabled){
//     console.log('THHHHHHH',moderatorIds[index])
//     return {
//         _participantID: moderatorIds[index]
//     }; 
// }
