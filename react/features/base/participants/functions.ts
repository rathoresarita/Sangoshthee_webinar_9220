...
export function getModerator(stateful: IStateful): IParticipant[] {
    const participants = toState(stateful)['features/base/participants'].remote;
    const moderators: IParticipant[] = [];

    // Iterate through the participants and find those with the role set to 'moderator'
    for (const participant of participants.values()) {
        if (participant.role === 'moderator'||participant?.fakeParticipant === 'RemoteScreenShare'|| participant?.fakeParticipant === "SharedVideo") {
            moderators.push(participant);
        }
    }
    console.log('get all moderaotrs',moderators)
...
export function getModeratorById(stateful: IStateful, id: string): IParticipant | undefined {
    const state = toState(stateful)['features/base/participants'];
    const { local, localScreenShare, remote } = state;

    console.log('remote.get(id).......................', remote.get(id));
    console.log('local?.id === id ? local : undefined', local?.id === id ? local : undefined);
    console.log('localScreenShare?.id === id ? localScreenShare : undefined', localScreenShare?.id === id ? localScreenShare : undefined);

    const remoteParticipant = remote.get(id);

    if (remoteParticipant && remoteParticipant.role === 'moderator'|| remoteParticipant?.fakeParticipant==='RemoteScreenShare' || remoteParticipant?.fakeParticipant==='SharedVideo') {
        return remoteParticipant  || (local?.id === id ? local : undefined)
        || (localScreenShare?.id === id ? localScreenShare : undefined);
;
    }
...
export function getRemoteModeratorsCount(stateful: IStateful) {
    const state = toState(stateful);
    const remoteParticipants = state['features/base/participants'].remote;

    // Assuming that the role information is stored in the participant data
    // const moderatorParticipants = new Map<string, IParticipant>(
    //     Array.from(remoteParticipants).filter(([id, participant]) => participant.role === 'moderator')
    // );
    // Assuming that the role information is stored in the participant data
const moderatorParticipants = new Map<string, IParticipant>(
    [...remoteParticipants].filter(([id, participant]) => participant.role === 'moderator')
);

    return moderatorParticipants.size;
}
