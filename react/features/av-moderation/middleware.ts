...
import { getConferenceState, getIsWebinarEnabled} from '../base/conference/functions';
...
import {
    ...
    REQUEST_ENABLE_AUDIO_MODERATION_FOR_WEBINAR
} from './actionTypes';
import {
    disableModeration,
...
MiddlewareRegistry.register(({ dispatch, getState }) => next => action => {
    ...
    const webinarEnabled=getIsWebinarEnabled(getState())
    ...
    case LOCAL_PARTICIPANT_MODERATION_NOTIFICATION: {
        ...
        dispatch(showNotification({
            ...
            customActionHandler: [ () => batch(() => {
            {(!webinarEnabled && !raisedHand)? dispatch(raiseHand(true)):null}
                dispatch(hideNotification(uid));
            }) ],
            descriptionKey,
            sticky: true,
    ...
    case REQUEST_ENABLE_AUDIO_MODERATION_FOR_WEBINAR:{
