...
export  const isEnableWebinarModeVisible=(state:IReduxState)=>{
    const  isLocalModerator=isLocalParticipantModerator(state)
    const inBreakoutRoom =isInBreakoutRoom(state);
    const {hideWebinarModeButton}=getParticipantsPaneConfig(state)
    return inBreakoutRoom?false :!hideWebinarModeButton &&isLocalModerator;
}
...
export function getParticipantPaneButtonProps(state: IReduxState) {
    const isModerator = isLocalParticipantModerator(state);
  
const webinarEnabled = state['features/base/conference'].webinarEnabled;
   //   // If webinar mode is enabled and the participant is not a moderator, hide the participant pane button.
   // // Otherwise, show the participant pane button to moderators and non-moderators as before.
  
   let visible=!webinarEnabled? true:isModerator 
  // let { isOpen } = state['features/participants-pane'];
//let  open= visible &&(isModerator? isOpen:!isOpen)

   
   console.log('visibleP',visible)
